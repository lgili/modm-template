 ...
 *     TimerChannel4::Interrupt_t flags = TimerChannel4::getInterruptFlags();
 * }
 * \endcode
 *
 * @author Jeff McBride
 ...
 */
class TimerChannel4 : public TimerChannelBase
{
public:
	...
	initialize()
	{
		ClockGen::enable<ClockPeripheral::Tc4>();
	}
	...
	setClockSource(ClockSource src)
	{
		if (src == ClockSource::Mck) {
			TC1->TcChannel[1].TC_EMR |= TC_EMR_NODIVCLK;
			return;
		} else {
			uint32_t tmp = TC1->TcChannel[1].TC_CMR & ~(TC_CMR_TCCLKS_Msk);
			tmp |= (uint32_t)src << TC_CMR_TCCLKS_Pos;
			TC1->TcChannel[1].TC_CMR = tmp;
	...
	getClockSource()
	{
	if (TC1->TcChannel[1].TC_EMR & TC_EMR_NODIVCLK) {
		return ClockSource::Mck;
	}
		const uint32_t cmr = TC1->TcChannel[1].TC_CMR;
	...
	{
		uint32_t cmr = TC1->TcChannel[1].TC_CMR;
		cmr &= ~TC_CMR_WAVSEL_Msk;
		cmr |= (uint32_t)ws << TC_CMR_WAVSEL_Pos;
		TC1->TcChannel[1].TC_CMR = cmr;
	}

	static inline void
	enable()
	{
		TC1->TcChannel[1].TC_CCR = TC_CCR_CLKEN;
	}

	static inline void
	disable()
	{
		TC1->TcChannel[1].TC_CCR = TC_CCR_CLKDIS;
	}
	...
	getValue()
	{
		return TC1->TcChannel[1].TC_CV;
	}

	static inline void
	setValue(uint16_t value)
	{
		TC1->TcChannel[1].TC_CV = value;
	}

	static inline uint16_t
	getRegA()
	{
		return TC1->TcChannel[1].TC_RA;
	}

	static inline void
	setRegA(uint16_t value)
	{
		TC1->TcChannel[1].TC_RA = value;
	}

	static inline uint16_t
	getRegB()
	{
		return TC1->TcChannel[1].TC_RB;
	}

	static inline void
	setRegB(uint16_t value)
	{
		TC1->TcChannel[1].TC_RB = value;
	}

	static inline uint16_t
	getRegC()
	{
		return TC1->TcChannel[1].TC_RC;
	}

	static inline void
	setRegC(uint16_t value)
	{
		TC1->TcChannel[1].TC_RC = value;
	}
	...
	{
		TC1->TcChannel[1].TC_IER = interrupts.value;
	}
	...
	{
		TC1->TcChannel[1].TC_IDR = interrupts.value;
	}
	...
	{
		return static_cast<Interrupt_t>(TC1->TcChannel[1].TC_SR & 0xf);
	}

	/** Generate a software trigger event */
	static inline void softwareTrigger()
	{
		TC1->TcChannel[1].TC_CCR = TC_CCR_SWTRG;
	}
	...
	{
		if(enabled) {
			TC1->TcChannel[1].TC_CMR |= TC_CMR_WAVE;
		} else {
			TC1->TcChannel[1].TC_CMR &= ~TC_CMR_WAVE;
		}
	}
	...
	{
		uint32_t tmp = TC1->TcChannel[1].TC_CMR;
