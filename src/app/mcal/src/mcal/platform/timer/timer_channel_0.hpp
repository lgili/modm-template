...
/**
 * @brief 		Timer Channel 0
 *
 * Driver for TC0 Channel 0
 *
 ...
 *     TimerChannel0::Interrupt_t flags = TimerChannel0::getInterruptFlags();
 * }
 * \endcode
 *
 * @author Jeff McBride
 ...
 */
class TimerChannel0 : public TimerChannelBase
{
public:
	...
	initialize()
	{
		ClockGen::enable<ClockPeripheral::Tc0>();
	}
	...
	setClockSource(ClockSource src)
	{
		if (src == ClockSource::Mck) {
			TC0->TcChannel[0].TC_EMR |= TC_EMR_NODIVCLK;
			return;
		} else {
			uint32_t tmp = TC0->TcChannel[0].TC_CMR & ~(TC_CMR_TCCLKS_Msk);
			tmp |= (uint32_t)src << TC_CMR_TCCLKS_Pos;
			TC0->TcChannel[0].TC_CMR = tmp;
	...
	getClockSource()
	{
	if (TC0->TcChannel[0].TC_EMR & TC_EMR_NODIVCLK) {
		return ClockSource::Mck;
	}
		const uint32_t cmr = TC0->TcChannel[0].TC_CMR;
	...
	{
		uint32_t cmr = TC0->TcChannel[0].TC_CMR;
		cmr &= ~TC_CMR_WAVSEL_Msk;
		cmr |= (uint32_t)ws << TC_CMR_WAVSEL_Pos;
		TC0->TcChannel[0].TC_CMR = cmr;
	}

	static inline void
	enable()
	{
		TC0->TcChannel[0].TC_CCR = TC_CCR_CLKEN;
	}
	...
	getValue()
	{
		return TC0->TcChannel[0].TC_CV;
	}

	static inline void
	setValue(uint16_t value)
	{
		TC0->TcChannel[0].TC_CV = value;
	}

	static inline uint16_t
	getRegA()
	{
		return TC0->TcChannel[0].TC_RA;
	}

	static inline void
	setRegA(uint16_t value)
	{
		TC0->TcChannel[0].TC_RA = value;
	}

	static inline uint16_t
	getRegB()
	{
		return TC0->TcChannel[0].TC_RB;
	}

	static inline void
	setRegB(uint16_t value)
	{
		TC0->TcChannel[0].TC_RB = value;
	}

	static inline uint16_t
	getRegC()
	{
		return TC0->TcChannel[0].TC_RC;
	}

	static inline void
	setRegC(uint16_t value)
	{
		TC0->TcChannel[0].TC_RC = value;
	}
	...
	static inline void enableInterruptVector(bool enable, uint32_t priority=5)
	{
		if(enable)
		{
			NVIC_SetPriority(TC0_IRQn, priority);
	...
	{
		TC0->TcChannel[0].TC_IER = interrupts.value;
	}
	...
	{
		TC0->TcChannel[0].TC_IDR = interrupts.value;
	}
	...
	{
		return static_cast<Interrupt_t>(TC0->TcChannel[0].TC_SR & 0xf);
	}

	/** Generate a software trigger event */
	static inline void softwareTrigger()
	{
		TC0->TcChannel[0].TC_CCR = TC_CCR_SWTRG;
	}
	...
	{
		if(enabled) {
			TC0->TcChannel[0].TC_CMR |= TC_CMR_WAVE;
		} else {
			TC0->TcChannel[0].TC_CMR &= ~TC_CMR_WAVE;
		}
	}
	...
	{
		uint32_t tmp = TC0->TcChannel[0].TC_CMR;
