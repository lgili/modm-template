...
public:
	...
	connect()
	{
		...
		using Connector = typename Pin::template Connector<Peripherals::Afec<1>, Peripherals::Afec<1>::Ad>;
	...
	initialize()
	{
		...
		ClockGen::enable<ClockPeripheral::Afec1>();
	...
	enableInterruptVector(bool enable, uint32_t priority=1)
	{
		if(enable)
		{
			NVIC_SetPriority(AFEC1_IRQn, priority);
	...
	getPinChannel()
	{
		using Connector = typename Pin::Ad::template Connector<Peripherals::Afec<1>, Peripherals::Afec<1>::Ad>;
