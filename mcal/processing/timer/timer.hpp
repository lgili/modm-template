...
namespace mcal::processing
{
  ...
  public:
    template <typename other_tick_type>
    static constexpr tick_type microseconds(other_tick_type value_microseconds) { return value_microseconds; }
    template <typename other_tick_type>
    static constexpr tick_type milliseconds(other_tick_type value_milliseconds) { return static_cast<tick_type>(1000UL) * microseconds(value_milliseconds); }
    template <typename other_tick_type>
    static constexpr tick_type seconds(other_tick_type value_seconds) { return static_cast<tick_type>(1000UL) * milliseconds(value_seconds); }
    template <typename other_tick_type>
    static constexpr tick_type minutes(other_tick_type value_minutes) { return static_cast<tick_type>(60UL) * seconds(value_minutes); }
    template <typename other_tick_type>
    static constexpr tick_type hours(other_tick_type value_hours) { return static_cast<tick_type>(60UL) * minutes(value_hours); }
    template <typename other_tick_type>
    static constexpr tick_type days(other_tick_type value_days) { return static_cast<tick_type>(24UL) * hours(value_days); }
    template <typename other_tick_type>
    static constexpr tick_type weeks(other_tick_type value_weeks) { return static_cast<tick_type>(7UL) * days(value_weeks); }
  ...
  private:
    ...
    {
      return static_cast<tick_type>(modm::PreciseClock::now().time_since_epoch().count());
    }
  };
}
